# PROMPT PHÂN TÍCH & KHUYẾN NGHỊ TRADING — THỊ TRƯỜNG VIỆT NAM (v7.3)

> **v7.3 — Phục hồi đầy đủ tri thức quốc tế + Web fetch đa nguồn**
> 
> So với v7.2, bản này KHÔI PHỤC đầy đủ các phần phân tích nền tảng
> đã có trong các iteration trước nhưng bị nén quá mức:
> - Sentiment thị trường (Composite Sentiment Score) trong Gate 1
> - Phân tích ngành đầy đủ với bảng 4 yếu tố trong Gate 2
> - 9 loại câu chuyện đầy đủ + 5 chiều priced in
> - Auction Market Theory (Balance vs Imbalance)
> - Volume Profile (POC, VAH, VAL, HVN, LVN) trong Gate 3
> - Order Flow + Behavioral Finance flags
> - 5 setups đầy đủ + chọn theo Regime
> - Statistical Edge (Sharpe + Expectancy + R + Win Rate)
> - Kelly Criterion (Quarter Kelly) + 3 cách position sizing
>
> Triết lý cốt lõi: ÁP DỤNG TRI THỨC QUỐC TẾ (chưa phổ biến tại VN)
> vào quyết định đầu tư, đồng thời chống lỗi giá cũ bằng web fetch
> đa nguồn nghiêm ngặt.
>
> Đặc trưng kỹ thuật:
> - Phase 0: xác định ngày giao dịch gần nhất (TARGET_DATE)
> - Phase 1: Web fetch đa nguồn (cafef + tradingview/investing/simplize)
> - 4 Convergence Gates: Vĩ mô → Ngành+Câu chuyện → Giá+Setup → Audit
> - Audit 8 câu hỏi cho mỗi mã candidate
> - JSON output schema phẳng tương thích hệ thống tự động
> - DEFAULT là KHÔNG KHUYẾN NGHỊ — chỉ khuyến nghị khi 4 Gates hội tụ
...
### Triết lý thiết kế
Áp dụng tri thức trading quốc tế (chưa phổ biến tại VN) vào quyết định
đầu tư, kết hợp với cơ chế chống ảo giác giá nghiêm ngặt và cơ chế
"default = không khuyến nghị" khi tín hiệu không hội tụ.

### 10 tri thức quốc tế được tích hợp

| # | Tri thức | Vị trí trong prompt |
|---|----------|---------------------|
| 1 | Order Flow & Microstructure | Gate 3 G3.4 |
| 2 | Volume Profile (POC, VAH, VAL, HVN, LVN) | Gate 3 G3.4 |
| 3 | Auction Market Theory (Balance/Imbalance) | Gate 1 G1.3 |
| 4 | Kelly Criterion + Position Sizing | Gate 3 G3.8 |
| 5 | Mean Reversion vs Momentum | Gate 3 G3.5 (chọn theo regime) |
| 6 | Regime Detection (4 trạng thái) | Gate 1 G1.2 |
| 7 | Statistical Edge (Sharpe + Exp + R + WR) | Gate 3 G3.7 |
| 8 | Behavioral Finance (Disposition, Anchoring, Herding) | Gate 1 G1.7 + Gate 3 G3.4 |
| 9 | Sentiment Analysis (DK-CoT, CSS) | Gate 1 G1.6 |
| 10 | Intermarket Correlation Matrix | Gate 1 G1.4 |

Cộng với đặc trưng VN: phát hiện mã có CÂU CHUYỆN RIÊNG (Gate 2)
+ đánh giá mức độ priced in (5 mức A-E theo 5 chiều).

### 4 Convergence Gates

| Gate | Tên | Pass/Fail logic |
|------|-----|------------------|
| Gate 1 | Vĩ mô + Regime + Sentiment + Intermarket | Điểm ≥ +4 / Regime không là 4 |
| Gate 2 | Phân tích ngành + Câu chuyện riêng + Lọc rủi ro | ≥ 2 mã đạt mức A/B |
| Gate 3 | Xác thực giá + Volume Profile + Setup + Edge | 5 checks pass + Sharpe ≥ 1.5 + R ≥ 2 |
| Gate 4 | Audit 8 câu hỏi | 8/8 hoặc 7/8 hợp lệ |

Mỗi Gate FAIL → KHÔNG KHUYẾN NGHỊ → KB3.

### Cơ chế chống ảo giác giá
- Phase 0: xác định TARGET_DATE bằng cách check thứ trong tuần + ngày lễ
- Phase 1: web fetch cafef + cross-check tradingview/investing/simplize
- 5 checks nghiêm ngặt cho mỗi mã ở Gate 3
- Câu hỏi 5 ở Gate 4 audit: lệch ≥ 2 ngày = LOẠI tuyệt đối

### JSON schema tương thích hệ thống tự động
JSON dùng schema phẳng. Logic 4 Gates ánh xạ qua các trường:
- `conclusion`: KB1/KB2/KB3 dựa trên gates pass
- `stand_aside_reason`: lý do nếu KB3
- `setup_confidence`: HIGH = audit 8/8, MEDIUM = 7/8
- `confidence` (market_context): 1-10 dựa trên gates
- `funnel_summary.candidates_after_technical`: số mã pass Gate 3
- Số phần tử `recommendations`: số mã pass Gate 4
- KHÔNG dùng null cho trường bắt buộc — loại mã thay vì điền null
