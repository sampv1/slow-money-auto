# PROMPT PHÂN TÍCH & KHUYẾN NGHỊ TRADING — THỊ TRƯỜNG VIỆT NAM (v7.1)

> **v7.1 = v7 (Convergence Gates + Audit) + JSON schema tương thích hệ thống tự động**
> 
...
>
> Cập nhật v7.1: JSON output dùng schema phẳng tương thích hệ thống
> trading tự động hiện tại. Logic Gates ánh xạ vào các trường JSON
> (conclusion, setup_confidence, confidence, funnel_summary).
...
## Tóm tắt điểm khác biệt cốt lõi v7.1 vs v6
...
### 5. JSON schema tương thích hệ thống tự động
JSON output v7.1 dùng schema phẳng tương thích hệ thống trading tự
động hiện tại (giống v6 đã dùng). Logic 4 Gates được áp dụng vào
JSON qua các trường:
- `conclusion`: KB1/KB2/KB3 dựa trên gates_passed
- `stand_aside_reason`: lý do nếu KB3
- `setup_confidence`: HIGH = audit 8/8, MEDIUM = 7/8
- `confidence` (market_context): 1-10 dựa trên gates pass
- `funnel_summary.candidates_after_technical`: số mã pass Gate 3
- Số phần tử `recommendations`: số mã pass Gate 4
- KHÔNG dùng null cho trường bắt buộc — loại mã thay vì điền null

### 6. funnel_summary trong JSON
