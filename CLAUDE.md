...
- **Dashboard data cache:** public-page Supabase reads are wrapped in `unstable_cache` (`dashboard/src/lib/cached-data.ts`) tagged `ta-data` / `fa-data` / `macro-data` (1 h TTL safety net). Freshness is event-driven: each pipeline workflow's last step POSTs `/api/revalidate?secret=…&tags=…` (secret = `REVALIDATE_SECRET`, set in both Vercel env and GitHub secrets), which expires the tags so the next view refetches. New page reads that hit pipeline-written tables should go through this lib and reuse the tags. A browser reload does NOT bypass the cache.
  ...
  - Functions are pinned to `sin1` (`dashboard/vercel.json`) to sit near Supabase + VN users; the default `iad1` crossed the Pacific on every miss. `x-vercel-id: <edge>::<function-region>` shows where a request actually ran.
