# PROMPT PHÂN TÍCH & KHUYẾN NGHỊ TRADING — THỊ TRƯỜNG VIỆT NAM (v4)

> **v4 bổ sung đầy đủ 10 tri thức trading quốc tế:**
> 1. Order Flow & Market Microstructure
> 2. Volume Profile (POC, VAH, VAL, HVN, LVN)
> 3. Auction Market Theory (Balance/Imbalance, Rotation/Trend)
> 4. Kelly Criterion & Position Sizing nâng cao
> 5. Mean Reversion vs Momentum (chọn theo regime)
> 6. Regime Detection (4 trạng thái)
> 7. Statistical Edge (Expectancy, Win Rate, R-Multiple, Sharpe)
> 8. Behavioral Finance (Disposition, Anchoring, Herding)
> 9. Intraday Seasonality (pattern phiên VN)
> 10. Correlation & Intermarket Analysis (cụ thể cho từng ngành)
>
...
• Thứ tự: A → B → C → D → E → F (12 bước) → G → H → I → J
...
## Bảng đối chiếu 10 tri thức quốc tế với v4

| # | Tri thức | Vị trí trong prompt |
|---|----------|----------------------|
| 1 | Order Flow & Microstructure | Bước 6 (chi tiết), Tầng 4 (Microstructure Flags) |
| 2 | Volume Profile (POC/VA/HVN/LVN) | Bước 5 (chi tiết đầy đủ) |
| 3 | Auction Market Theory | Phần A.2 (Balance vs Imbalance) + Bước 4 |
| 4 | Kelly Criterion | Bước 12 (3 cách tính, Quarter Kelly) |
| 5 | Mean Rev vs Momentum | Phần D Tầng 3 (chọn setup theo regime) |
| 6 | Regime Detection | Phần A.1 (4 regime + công cụ xác định) |
| 7 | Statistical Edge | Phần E (Sharpe + Expectancy + R-Multiple + WR) + Phần J (track record) |
| 8 | Behavioral Finance | Tầng 4 (Behavioral Flags) + Bước 7 (chi tiết) |
| 9 | Intraday Seasonality | Phần C (bảng pattern phiên VN) + Kế hoạch vào lệnh |
| 10 | Intermarket & Correlation | Phần B.2 (ma trận correlation cụ thể) + Bước 8 |

## Thay đổi chính so với v3

- **Thêm Auction Market Theory** (Phần A.2) — khái niệm Balance/Imbalance
- **Thêm Intraday Seasonality** (Phần C) — bảng pattern phiên VN chi tiết
- **Thêm ma trận Correlation** cụ thể cho từng ngành (Phần B.2)
- **Thêm Behavioral Finance** (Tầng 4 + Bước 7 mới)
- **Thêm Statistical Edge** (Expectancy, R-Multiple, Win Rate, không chỉ Sharpe)
- **Thêm Kelly Criterion** chi tiết (Quarter Kelly, Volatility-based sizing)
- **Thêm Volume Profile chi tiết** (HVN, LVN, không chỉ POC/VA)
- **Thêm Order Flow chi tiết** (Delta, Aggressive/Passive, Iceberg)
- **Thêm Track Record** (Phần J) — học từ chính kết quả của mình
- **Mở rộng reasoning từ 10 bước thành 12 bước** — mỗi bước gắn với một tri thức cụ thể
...
3. **Chạy prompt Trading v3** với context bổ sung sentiment
4. **Đọc kỹ reasoning 10 bước** cho mỗi mã — nếu không logic hoặc thiếu dữ liệu, LOẠI
