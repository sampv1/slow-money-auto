...
export default async function SignalProPage({
  ...
  const { data: universe } = await fetchAllPaged<{
    ...
    (from, to) =>
      supabase
        .from("ta_universe")
        .select("symbol,avg_volume_20d,rs_3m,rs_composite,rs_line_full,rs_line_score,rs_line_grade,base_score,base_grade,base_type,base_status,ta_score,final_score")
        .range(from, to),
  );
