...
Metric:
  interbank_overnight — SBV "Lãi suất bình quân liên ngân hàng qua đêm" (%/năm),
      daily. Source: Vietstock macro NormID 293 (category 66 "Lãi suất liên ngân
      hàng VNIBOR"), via the same /Macro/GetReportDataByIDs endpoint used for the
      central rate. Vietstock republishes SBV's official interbank averages;
      SBV's own statistics portal firewall-blocks cloud IPs (its legacy JSPX
      routes), so Vietstock is the cloud-reachable path (verified GitHub
      Actions-safe, same host as the central rate + CPI feeds).
...
CURRENCY: current, unlike the frozen CPI feed. It lags the equity market by a few
business days, which is normal for interbank-rate publication.
"""
